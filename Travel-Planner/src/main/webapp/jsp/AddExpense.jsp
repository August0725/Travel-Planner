...
<form action="/itinerary/expenseLog/${tripId}/${itinerary.getId()}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        $<input type="number" name="transportationExpense" step="0.01" placeholder="Enter values" value="${expenseLog.getMealsExpense()}" required>
    </div>
    <div class="form-group">
        <label>Activities Fee</label>
        $<input type="number" name="accommodationExpense" step="0.01" placeholder="Enter values" value="${expenseLog.getActivitiesExpense()}" required>
    </div>
